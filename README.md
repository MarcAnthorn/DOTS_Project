# CrowdFlow — 基于 Unity DOTS 的高密度单位群体运动模拟
...
本项目以 RTS 中高密度单位同时移动的性能问题为切入点，使用 **Unity DOTS**（ECS + Jobs + Burst）构建数据驱动的群体运动系统。工程重点是把大规模移动模拟组织成可增量维护、可验证正确性的计算管线：单位位置与速度由 ECS 统一承载，寻路、接触检测与求解各自作为独立阶段运行在并行 Job 之上。

```
输入指令 → 方向场查询 → 接触候选维护 → 软避让与约束投影 → 位置速度写回
```
...
1. **全图方向场**：BFS 预计算方向场，单位移动仅需查表；采用三阶段烘焙与双缓冲发布，目的地变化时只重算距离与方向两层，旧地图在烘焙期间持续可用；到达区以滞回与减速逻辑收敛。
2. **增量接触维护**：跨帧保留候选接触关系，每帧只处理位置或速度变化的单位，变化比例过高时回退全量重建；调试构建中以独立全量扫描校验结果，防止碰撞漏检。
3. **缓存正确性保障**：缓存视图在消费前须通过证书校验（单位集合、求解参数、覆盖范围），校验失败立即回退，确保错误数据不会进入求解阶段。
4. **软避让与位置约束**：距离斥力与相对速度预判两种避让策略，叠加位置级约束投影；并行修正按固定顺序聚合，保证多线程结果可复现。
